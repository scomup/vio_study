...
class PreintegrationExample:
    ...
    def __init__(self,
        ...
        if bias is not None:
            ...
            accBias = np.array([0.01, 0.02, 0.05])
            gyroBias = np.array([0.01, 0.02, 0.05])
    ...
    def run(self, T: int = 12):
        ...
        for i, t in enumerate(np.arange(0, T, self.dt)):
            ...
            cur_pose = self.scenario.pose(t)
            imu.append(np.concatenate([np.array([t]),measuredAcc, measuredOmega]))
            if i % 25 == 0:
                gps.append(np.concatenate([np.array([t]),cur_pose.translation(), cur_pose.rotation().quaternion()]))
        ...
        np.save('/home/liu/bag/gps0.npy', np.array(gps))
        np.save('/home/liu/bag/imu0.npy', np.array(imu))
